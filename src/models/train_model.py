...
class ModelTrainer:
    ...
    def train_model(
        ...
        **kwargs,
        ...
        # Warn about any remaining missing values before fitting
        missing = X_train.isnull().sum()
        if missing.any():
            logger.warning("Missing values detected before fill:\n%s", missing[missing > 0])
    ...
    def get_feature_importance(
        self, model: Any, feature_names: list, top_n: int = 20
    ) -> pd.DataFrame:
        """
    ...
    def hyperparameter_tuning(
        ...
        grid_search = GridSearchCV(
            base_model, param_grid, cv=cv, scoring="neg_mean_squared_error", n_jobs=-1, verbose=1
        )

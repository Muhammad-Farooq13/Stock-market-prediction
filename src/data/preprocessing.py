...
class DataPreprocessor:
    ...
    def scale_features(self, X_train: pd.DataFrame, X_test: pd.DataFrame = None) -> Tuple:
        ...
        X_train_scaled = pd.DataFrame(
            X_train_scaled, columns=self.feature_names, index=X_train.index
        )
        ...
        if X_test is not None:
            ...
            X_test_scaled = pd.DataFrame(
                X_test_scaled, columns=self.feature_names, index=X_test.index
            )
            logger.info("Scaled test features")
            return X_train_scaled, X_test_scaled

        return X_train_scaled

    def create_time_series_splits(
        self, df: pd.DataFrame, date_column: str, train_size: float = 0.7, val_size: float = 0.15
    ) -> Tuple:
        """
    ...
    def preprocess_pipeline(
        self, df: pd.DataFrame, target_column: str = None, date_column: str = None
    ) -> dict:
        """
        ...
        if date_column and date_column in df_encoded.columns:
            # Time-based split
            train_df, val_df, test_df = self.create_time_series_splits(df_encoded, date_column)

            X_train = train_df.drop(
                columns=[target_column, date_column] if target_column else [date_column]
            )

...
class Visualizer:
    ...
    def plot_time_series(
        ...
        for col in value_columns:
            fig.add_trace(go.Scatter(x=df[date_column], y=df[col], mode="lines", name=col))
    ...
    def plot_predictions_vs_actual(
        ...
        """
        fig = make_subplots(
            rows=1, cols=2, subplot_titles=("Scatter Plot", "Time Series Comparison")
        )
        ...
        fig.add_trace(
            go.Scatter(x=indices, y=y_true, mode="lines", name="Actual", line=dict(color="blue")),
            row=1,
            col=2,
        )
        fig.add_trace(
            go.Scatter(x=indices, y=y_pred, mode="lines", name="Predicted", line=dict(color="red")),
            row=1,
            col=2,
        )
        ...
        fig.update_layout(title_text=title, showlegend=True, template="plotly_white", height=500)
    ...
    def plot_residuals(
        ...
        # Residuals vs predicted
        fig.add_trace(
            go.Scatter(x=y_pred, y=residuals, mode="markers", marker=dict(size=5, opacity=0.6)),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(go.Scatter(x=indices, y=residuals, mode="lines"), row=2, col=2)
        ...
        fig.update_layout(title_text=title, showlegend=False, template="plotly_white", height=800)
    ...
    def plot_candlestick(
        ...
        if volume_col:
            ...
            # Volume
            fig.add_trace(go.Bar(x=df[date_column], y=df[volume_col], name="Volume"), row=2, col=1)
